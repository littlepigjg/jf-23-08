...
import {
  type MarketCrisis,
  type CrisisGoodState,
  createInitialCrisisStates,
  updateCrisisStates,
  tickCrises,
  getCrisisPriceMultiplier,
  triggerChainReaction,
} from './crisisSystem';

export type { MarketCrisis, CrisisGoodState };
export {
  CRISIS_TRIGGER_DECLINE_TICKS,
  CRISIS_MIN_DURATION,
  CRISIS_MAX_DURATION,
  CRISIS_PRICE_DROP_MULTIPLIER,
  CRISIS_CHAIN_REACTION_CHANCE,
} from './crisisSystem';
...
export const createInitialMarketState = (
  ...
  const crisisStates = createInitialCrisisStates(GOODS.map((g) => g.id));
...
export const tickMarketStateWithEvents = (
  ...
  for (let i = 0; i < ticks; i++) {
    ...
    for (const gid of Object.keys(trends)) {
      trendValues[gid] = trends[gid].value;
    }
    const crisisUpdate = updateCrisisStates(crisisStates, trendValues, crises);
